...
class JobReccomendationApp
  ...
  def find_matches
    job_seekers.each do |jobseeker|
      jobs.each do |job|
        ...
        reccomendations << Reccomendation.new(jobseeker.id, jobseeker.name, job.id, job.title, matching_skills.count, matching_skills.count)
      end
    end
  end
  ...
  def csv_header = ["jobseeker id", "jobseeker name", "job id", "job title", "matching skill count", "matching skill percent"]
